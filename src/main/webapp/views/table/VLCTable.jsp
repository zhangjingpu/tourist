...
<script type="text/javascript">
    ...
    layui.use(['jquery',], function (args) {
        ...
        $.ajax({	//使用JQuery内置的Ajax方法
            ...
            url: "${pageContext.request.contextPath}/DataTwo/getVLCInfo.do",	//请求发送到ShowInfoIndexServlet处
            ...
            success: function (result) {
                ...
                if (result != null && result.length > 0) {
                    for (var i = 0; i < result.length; i++) {
                        ...
                        if (result[i].vlc > result[i].mq_v) {
                            layer.msg("警告！超过景区最大车流承载量！")
                        }
                        VLC.push(result[i].vlc);
                        VF.push(result[i].vf);
                    ...
                    option.series[0].data = VF;
                    option.series[1].data = VLC;
                ...
                success: function (result) {
                    ...
                    if (result != null && result.length > 0) {
                        if (a[a.length - 1].time_hour == (app.count) && app.count < 24) {
                            ...
                            app.count++;
                            //time.push(result[i].time_hour);		//挨个取出温度等值并填入前面声明的温度、湿度、压强等数组
                            var VLC = (a[a.length - 1].vlc);
                            var VF = (a[a.length - 1].vf);
                            var count_1 = (a[a.length - 1].time_hour);
                            //alert(result.length);
                            if (VLC > a[a.length - 1].mq_v) {
                                layer.msg("警告！超过景区最大车流承载量！")
                            }
                            myChart.hideLoading();	//隐藏加载动画
                            option.xAxis[0].data.push(count_1);
                            //option.xAxis[1].data.push(count_1);
                            option.series[0].data.push(VF);
                            option.series[1].data.push(VLC);
                            myChart.setOption(option);
                        }
                    }
                    else {
